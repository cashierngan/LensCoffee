...
const minimize = false;//ReactNative.isMinimizeEnabled({ fallback: !dev });
let devServer = ReactNative.getDevServerOptions();
devServer.hmr = false;
...
module.exports = {
    ...
    resolve: {
        ...
        alias: {
            // '@core': path.resolve(__dirname, '/src/define'),
            // '@components': path.resolve(__dirname, '/src/components'),
            // '@expense-context': path.resolve(__dirname, '/src/miniApp/vn.momo.expense/contexts'),
            ...
            // '@mini-app': path.resolve(__dirname, '/src/miniApp'),
        },
    },
    /**
     * Configures output.
        ...
        rules: [
            {
                ...
                include: [
                    ...
                    /node_modules(.*[/\\])+@momo-platform[/\\]api/,
                    /node_modules(.*[/\\])+@momo-platform[/\\]audiences/,
                    /node_modules(.*[/\\])+@momo-platform[/\\]eslint-plugin-momo/,
                    /node_modules(.*[/\\])+@momo-platform[/\\]momo-news-feed/,
                    /node_modules(.*[/\\])+@momo-platform[/\\]utils/,
                    /node_modules(.*[/\\])+@momo-platform[/\\]momo-cornerstone/,
                    /node_modules(.*[/\\])+@momo-kit/,
                ],
                use: 'babel-loader',
            },
            /**
    ...
    plugins: [
        ...
        new webpack.DllReferencePlugin({
            context: __dirname,
            name: 'self.momo_dll',
            manifest: require(`@momo-platform/momo-core/dll/main-manifest.${platform}.${mode}.json`),
        }),
        new webpack.DllReferencePlugin({
            context: __dirname,
            scope: 'momo-core',
